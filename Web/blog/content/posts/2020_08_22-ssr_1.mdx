...
object MySsr {
  ...
  val setup: Expr[Unit] =
    ReactSsr.Setup(
      // Here we use the development JS because it's made for Node and browsers
      // — the minified production JS only works with browsers since React 17.
      Expr.requireFileOnClasspath("react.development.js"),
      Expr.requireFileOnClasspath("react-dom-server.browser.development.js"),
...
```scala
  ...
  def doIt(input: MySpaInputs): Expr.Result[String] =
    init.flatMap(_ => ctx.eval(renderMySpa(input)))
```
