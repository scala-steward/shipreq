...
desc : In the JS world, React users can render their pages on the server side for a better user experience. Here's how to do in the Scala world.
tags : ["tech"]
---
...
* Embedded JS is still JS; it doesn't speak Scala, even though you're on the JVM, so you need a communication protocol.
...
* We create a `val setup`, which is a mini-program/function/procedure that we can execute in JS.
  We don't actually execute it here, just create it as an `Expr[Unit]`,
...
The `(_.asString)` function is where we process the JS function's output.
...
* The `Sync` in `ContextSync` means we interact with the context is a synchronous manner. <SG/>
  provides asynchronous versions as well.
...
create a new context, spend time loading React, any deps, and all of your Scala.JS into it,
and then when it finally gets around to calling your component, it will do so slowly
because the env/context/sandbox is new and cold (just like cold vs hot JVMs).
...
Ideally you'd want to warmup it with different inputs.
