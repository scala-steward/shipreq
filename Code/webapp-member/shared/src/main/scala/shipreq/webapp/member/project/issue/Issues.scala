...
import shipreq.webapp.member.project.data.{ReqCodeGroupId, ReqId}
...
final case class Issues(vector: Vector[Issue]) {
  ...
  def filter(f: CompiledFilter): Issues =
    Issues(vector.filter {
      ...
      case _: Issue.FieldFormulaRefsDeadFormulaField => false
  ...
  lazy val bySource: BySource = {
    ...
    vector.foreach {
      ...
      case i: Issue.FieldFormulaRefsDeadFormulaField => config = config.add(i)
