...
final class FieldSetRules(val imp    : CustomField.Implication.Id => Resolution.ForImpField,
                          val formula: CustomField.Formula    .Id => Resolution.ForFormulaField,
  ...
  def apply(id: FieldId): Resolution[Any] =
    id match {
      ...
      case i: CustomField.Formula.Id     => formula(i)
  ...
  override def hashCode =
    ((((imp.hashCode + tag.hashCode * 31) * 31 + text.hashCode) * 31 + static.hashCode) * 31 + num.hashCode) * 31 + formula.hashCode

  override def equals(obj: Any): Boolean =
    obj match {
      case f: FieldSetRules => (this eq f) || ((imp eq f.imp) && (formula eq f.formula) && (num eq f.num) && (tag eq f.tag) && (text eq f.text) && (static eq f.static))
      case _                => false
    }
}
...
object FieldSetRules {
  ...
  def apply(imp    : CustomField.Implication.Id => Resolution.ForImpField,
            formula: CustomField.Formula    .Id => Resolution.ForFormulaField,
    ...
    new FieldSetRules(imp, formula, num, tag, text, static)
