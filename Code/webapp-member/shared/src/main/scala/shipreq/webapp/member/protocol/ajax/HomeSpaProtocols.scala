...
object HomeSpaProtocols {
  ...
  object CreateUserGroup {
    final case class Request(name  : UserGroup.Name,
                             ...
                             invs  : Set[UserGroupInv.Target[Username]],
                            )
      ...
      val picklerRequest: Pickler[Request] =
        new Pickler[Request] {
          ...
          override def unpickle(implicit state: UnpickleState): Request = {
            ...
            val invs   = state.unpickle[Set[UserGroupInv.Target[Username]]]
            Request(name, handle, rels, invs)
          }
        }
  ...
  object UpdateUserGroup {
    final case class Request(id        : UserGroup.Id.Public,
                             ...
                             newInvs   : Set[UserGroupInv.Target[Username]],
                             revokeInvs: Set[UserGroupInv.Id.Public],
                            )
      ...
      val picklerRequest: Pickler[Request] =
        new Pickler[Request] {
          override def pickle(a: Request)(implicit state: PickleState): Unit = {
            ...
            state.pickle(a.name)
            state.pickle(a.handle)
            state.pickle(a.newInvs)
            state.pickle(a.revokeInvs)
          }
          override def unpickle(implicit state: UnpickleState): Request = {
            val id         = state.unpickle[UserGroup.Id.Public]
            ...
            val newInvs    = state.unpickle[Set[UserGroupInv.Target[Username]]]
            val revokeInvs = state.unpickle[Set[UserGroupInv.Id.Public]]
            Request(id, name, handle, rels, newInvs, revokeInvs)
          }
        }
