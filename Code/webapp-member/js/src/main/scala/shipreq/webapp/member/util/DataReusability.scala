...
import shipreq.webapp.member.event.{EventOrd, ProjectAndOrd}
import shipreq.webapp.member.filter.Filter.Implicits._
import shipreq.webapp.member.filter.{CompiledFilter, Filter}
import shipreq.webapp.member.issue.{Issue, Issues}
...
import shipreq.webapp.member.text.{Atom, PlainText, ProjectText, TextSearch}
