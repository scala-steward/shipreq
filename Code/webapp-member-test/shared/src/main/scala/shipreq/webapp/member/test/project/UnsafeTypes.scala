...
object MakeEmpty {
  ...
  implicit def emptyCustomNumberFieldGD: MakeEmpty[CustomNumberFieldGD.Values] = MakeEmpty(CustomNumberFieldGD.emptyValues)
...
//        .toVector
  ...
  implicit def autoVerifiedEventSeqFromOne(v: VerifiedEvent): VerifiedEvent.Seq =
    VerifiedEvent.Seq.one(v)
...
object UnsafeTypes extends UnsafeTypesMedPriority {
  ...
  implicit class UnsafeIntExt(val a: Int) extends AnyVal {
    ...
    def CFNum  = CustomField.Number.Id(a)
