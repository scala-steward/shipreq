...
//      new CrudActionGens(RandomData.tagId, tagCrudInput)
  ...
  object filter {
    ...
    import shipreq.webapp.member.project.filter.FilterAst.{FieldCriteria, Scope}
    import shipreq.webapp.member.project.filter.Filter._
    ...
    object potential {
      ...
      def fieldCriteria[A](ga: Option[Gen[A]]): Gen[Potential.FieldCriteriaF[A]] = {
        ...
        gens :+= Gen.double.map(FieldCriteria.LiteralNumber.apply)
        Gen.chooseGenNE(gens)
      }
