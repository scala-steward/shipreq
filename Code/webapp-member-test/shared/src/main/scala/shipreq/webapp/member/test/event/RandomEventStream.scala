...
import shipreq.webapp.member.test.DataTestExt._
import shipreq.webapp.member.test.RandomData
import shipreq.webapp.member.test.RandomData.{TextGen, TextGenExt, customReqTypeName, desc, exclusivity, fieldName, fieldRefKey, filter, filterDead, genColour, hashRefKey, implicationRequired, mandatory, reqCode, reqTypeMnemonic, tagGroupName}
...
final class ApplicableEventGen(curState: State, config: RandomEventStreamConfig) {
  ...
  object savedViewGDv1 extends GenericDataGen(SavedViewGDv1) {
    import gd._
    import shipreq.webapp.member.data.savedview.Column
