...
class MailChimpTest extends Specification {
  ...
  def p[R](f: JValue => ErrorOr[R], txt: String): R =
    ErrorOr.require_!(parseJson(txt) >=> f)
  ...
  "Total API failures" >> {
    ...
    "parseErrorResponse" in {
      parseErrorResponse(
        """{"status":"error","code":553,"name":"Invalid_PagingLimit","error":"Page Limit Number must be greater than or equal to 0"}"""
      ) must beNonErrorOf(TotalApiFailure(553, "Invalid_PagingLimit", "Page Limit Number must be greater than or equal to 0"))
    }
  }
  ...
  "lists/list" >> {
    "ok" in {
      ...
      ) must beNonErrorAnd(beSome(ListId("270dff4105")))
    }

    "no match" in {
      processResponse(GetListId(""))("""{"total":0,"data":[],"errors":[]}""") must beNonErrorAnd(beNone)
    }
  }

  "lists/batch-subscribe" >> {
    "new user" in {
      processResponse(BatchSubscribe(null, null))(
