package shipreq.webapp.base

/** Generated by webtamp. */
object AbstractAssetManifest {
  final case class CDN(href: String, integrity: Option[String])
}

abstract class AbstractAssetManifest[+A] {
  import AbstractAssetManifest._

  protected def modify(f: String): A

  final val analyticsJs = modify("/assets/analytics.js")

  final val favicon16X16Png = modify("/assets/favicon-16x16.png")

  final val favicon32X32Png = modify("/assets/favicon-32x32.png")

  final val faviconAndroidChrome192X192Png = modify("/assets/android-chrome-192x192.png")

  final val faviconAndroidChrome512X512Png = modify("/assets/android-chrome-512x512.png")

  final val faviconAppleTouchIconPng = modify("/assets/apple-touch-icon.png")

  final val faviconBrowserconfigXml = modify("/assets/browserconfig.xml")

  final val faviconIco = modify("/assets/favicon.ico")

  final val faviconMstile144X144Png = modify("/assets/mstile-144x144.png")

  final val faviconMstile150X150Png = modify("/assets/mstile-150x150.png")

  final val faviconMstile310X150Png = modify("/assets/mstile-310x150.png")

  final val faviconMstile310X310Png = modify("/assets/mstile-310x310.png")

  final val faviconMstile70X70Png = modify("/assets/mstile-70x70.png")

  final val faviconSafariPinnedTabSvg = modify("/assets/safari-pinned-tab.svg")

  final val faviconSiteWebmanifest = modify("/assets/site.webmanifest")

  final val jqueryJs = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/jquery/2.2.4/jquery.min.js",
    integrity = Some("sha256-BbhdlvQf/xTY9gja0Dq3HiwQF8LaCRTXxZKRutelT44="))

  final val katexCss = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/KaTeX/0.11.1/katex.min.css",
    integrity = Some("sha256-V8SV2MO1FUb63Bwht5Wx9x6PVHNa02gv8BgH/uH3ung="))

  final val katexJs = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/KaTeX/0.11.1/katex.min.js",
    integrity = Some("sha256-F/Xda58SPdcUCr+xhSGz9MA2zQBPb0ASEYKohl8UCHc="))

  final val loadingSpinSvg = modify("data:image/svg+xml;base64,")

  final val loadjs = modify("/assets/loadjs.js")

  final val memberLibBundleJs = modify("/assets/member-lib-bundle.js")

  final val prismJsAutoloader = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/plugins/autoloader/prism-autoloader.min.js",
    integrity = Some("sha256-3S2PESHNt0YNL65z57WuHPHIv12fibpBDXepyCGHftw="))

  final val prismJsCore = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/components/prism-core.min.js",
    integrity = Some("sha256-9h14mWYYiQGkeAKg2JtijbqApb56kgw57WN6sI6dwH0="))

  final val prismJsCss = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/themes/prism-okaidia.css",
    integrity = Some("sha256-PCIRBVLXb7L62+EomqoQYE4fCb7LpmGlAcPrbnbRAyw="))

  final val prismJsLineNumbers = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/plugins/line-numbers/prism-line-numbers.min.js",
    integrity = Some("sha256-hep5s8952MqR7Y79JYfCXZD6vQjVHs7sOu/ZGrs1OEQ="))

  final val prismJsLineNumbersCss = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/plugins/line-numbers/prism-line-numbers.css",
    integrity = Some("sha256-udLi8HM3vM4cuDDMGyRFzG8ek0UN0+uytPLWkbTpagg="))

  final val prismJsMatchBraces = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/plugins/match-braces/prism-match-braces.min.js",
    integrity = Some("sha256-ICsqq6gRT8YxNMWqiSlCk+Xxr8XpkSaSVW+apneJuYs="))

  final val prismJsMatchBracesCss = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/prism/1.20.0/plugins/match-braces/prism-match-braces.css",
    integrity = Some("sha256-Qr3kZtoWj5+ax8ifwd0asqiDHBSyxWOKltSh2AH2XBA="))

  final val reactDomJs = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/react-dom/16.13.1/umd/react-dom.development.js",
    integrity = Some("sha256-/z9BuuCDG5rQaHQBQWtzk3dBOTjTAOrn+SUSRGDjR40="))

  final val reactDomServerJs = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/react-dom/16.13.1/umd/react-dom-server.browser.development.js",
    integrity = Some("sha256-e8L+Ctskvl/QseYUB0FNyOexbyaeDBCx9O9jAvXs4I0="))

  final val reactJs = CDN(
    href = "https://cdnjs.cloudflare.com/ajax/libs/react/16.13.1/umd/react.development.js",
    integrity = Some("sha256-4gJGEx/zXAxofkLPGXiU2IJHqSOmYV33Ru0zw0TeJ30="))

  final val semanticCss = modify("/assets/semantic.css")

  final val semanticCssUrls: List[A] =
    modify("icons.eot") ::
    modify("icons.eot?#iefix") ::
    modify("icons.woff2") ::
    modify("icons.woff") ::
    modify("icons.ttf") ::
    modify("icons.svg#icons") ::
    Nil

  final val semanticJs = modify("/assets/semantic.js")

  final val shipreqBannerSvg = modify("/assets/shipreq-banner.svg")

  final val shipreqCircleDarkSvg = modify("data:image/svg+xml;base64,")

  final val shipreqLogoSvg = modify("data:image/svg+xml;base64,")

  final val sortAscSvg = modify("data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIyMCIgaGVpZ2h0PSIyMCIgdmlld0JveD0iMCAwIDIwIDIwIj48cGF0aCBkPSJNLjUgMjBMMTAgMGw5LjUgMjAiLz48L3N2Zz4=")

  final val sortBlankSvg = modify("data:image/svg+xml;base64,")

  final val tileExclamationSvg = modify("data:image/svg+xml;base64,")

  final val tileQuestionSvg = modify("data:image/svg+xml;base64,")

  final val vizJs = modify("/assets/viz.js")

  final val vizWasm = modify("/assets/viz.wasm")
}