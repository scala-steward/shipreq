...
object Rev4 {
  ...
  implicit lazy val pickleValidFilter: Pickler[Filter.Valid] = {
    ...
    implicit val picklerFilterAstFieldProp: Pickler[FilterAst.FieldProp[Valid.Field, Valid.FieldCriteria]] =
      new Pickler[FilterAst.FieldProp[Valid.Field, Valid.FieldCriteria]] {
        override def pickle(a: FilterAst.FieldProp[Valid.Field, Valid.FieldCriteria])(implicit state: PickleState): Unit = {
          state.pickle(a.field)
          state.pickle(a.criteria)
        }
        override def unpickle(implicit state: UnpickleState): FilterAst.FieldProp[Valid.Field, Valid.FieldCriteria] = {
          val field = state.unpickle[Valid.Field]
          val attr  = state.unpickle[Valid.FieldCriteria]
      ...
      new Pickler[ValidF[Unit]] {
        ...
        override def pickle(a: ValidF[Unit])(implicit state: PickleState): Unit =
          a match {
            ...
            case b: FilterAst.FieldProp     [Valid.Field, Valid.FieldCriteria] => state.enc.writeByte(KeyFieldProp     ); state.pickle(b)
          }
        override def unpickle(implicit state: UnpickleState): ValidF[Unit] =
          state.dec.readByte match {
            ...
            case KeyFieldProp      => state.unpickle[FilterAst.FieldProp     [Valid.Field, Valid.FieldCriteria]]
          }
      }
