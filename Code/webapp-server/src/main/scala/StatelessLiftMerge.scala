// This is a copy of LiftMerge.scala in Lift, modified so that
// it doesn't insert Lift-specific and session-related JS into its output.
...
//  }
  ...
  def merge(xhtml: NodeSeq, req: Req): Node = {
    val snippetHashs: HashMap[String, Box[NodeSeq]] = self.deferredSnippets.is
...
//        }
      ...
      htmlKids += nl
      htmlKids += bodyElement.copy(child = bodyChildren.toList) // htmlKids += bodyAttributes.foldLeft(bodyElement.copy(child = bodyChildren.toList))(_ % _)
      htmlKids += nl
      ...
      val ret: Node = if (Props.devMode) {
        LiftRules.xhtmlValidator.toList.flatMap(_(tmpRet)) match {
          ...
          case xs =>
            ...
            (new RuleTransformer(rule)).transform(tmpRet).head
        }
