...
import shipreq.webapp.server.feature.uc.{SavedSteps, UcParsingCtx}
import shipreq.webapp.server.feature.uc.change._
import shipreq.webapp.server.feature.uc.field.{NormalCourseField, StepField}
import shipreq.webapp.server.feature.uc.text.ParsingConfig.{FlowToStyle, FlowFromStyle, makeInvalidStepRef}
import shipreq.webapp.server.lib.Misc.SingleSpace
...
object StepText {
  def correctInput(input: String): InputCorrected[String] =
    Validators.usecase.stepFieldText.correctedU(input)
