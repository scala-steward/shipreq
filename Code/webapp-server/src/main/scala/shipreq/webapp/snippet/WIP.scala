...
class WIP {
  ...
  def newProject = {
    ...
    val List(p1,p3,p5) = List[ApplicableTag.Id](4,3,2)
    val (p2,p4) = (p3,p5)
    val mfs = (0 to 28).toVector.map(GenericReq.Id(_))

    val contentByDsl = (
      GReq(reqType = mf, id = mfs( 1), desc = "Use Case Editor").tag(p5) // MF-1
    + GReq(reqType = mf, id = mfs( 2), desc = "Anonymous Share").tag(p2) // MF-2
    ...
    + GReq(reqType = mf, id = mfs( 5), desc = "Field Customisation").tag(p5) // MF-5
    + GReq(reqType = mf, id = mfs( 6), desc = "Incompletions").tag(p3) // MF-6
    + GReq(reqType = mf, id = mfs( 7), desc = "Organisation").tag(p5) // MF-7
    ...
    + GReq(reqType = mf, id = mfs(12), desc = "Low-level Requirements (CO, FR, BR, etc.)").tag(p5) // MF-12
    + GReq(reqType = mf, id = mfs(13), desc = "Requirement Relationships").tag(p4) // MF-13
    ...
    + GReq(reqType = mf, id = mfs(22), desc = "High-level Requirements").tag(p3) // MF-22
