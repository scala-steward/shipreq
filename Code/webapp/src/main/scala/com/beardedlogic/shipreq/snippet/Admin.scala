...
import SystemInfo.{Build, RunModeShort}
...
object Admin extends SnippetHelpers {

  // TODO Overlap between snippets: Admin, Prop, SystemInfo
  ...
  def allStats: List[(String, List[(String, StatValue)])] = daoProvider.withAdminDao(dao => {
    ...
    List(
      "System & Environment" -> List(
        ...
        , "run.mode.full" -> Str(Props.mode.toString)
        , "run.mode.short" -> Str(RunModeShort)
