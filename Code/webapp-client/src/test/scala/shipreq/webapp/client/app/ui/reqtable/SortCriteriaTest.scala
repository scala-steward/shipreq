...
object SortCriteriaTest extends TestSuite {
  ...
  */
  ...
  val wantTest = for {
    cfs  ← RandomReqTableData.customFieldColumn.vector
    gi   = RandomReqTableData.ColumnIGen(cfs)
    scis ← gi.sortCriIs
    ci   ← gi.columnI
    cc   ← RandomReqTableData.columnC
    sc   ← RandomReqTableData.sortCriteria(scis)
