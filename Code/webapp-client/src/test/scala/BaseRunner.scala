    ...
    utest.runSuite(
      ...
      logFailure = (msg, thrown) => {
        ...
        // since the user probably doesn't care about those anyway
          // thrown.getStackTrace.take(3).map(_.getClassName) foreach println; println()
          // thrown.getStackTrace.take(100).map(_.getClass.getName) foreach println; println()
          // thrown.getStackTrace.take(10).map(_.getClass.getSimpleName) foreach println; println()
