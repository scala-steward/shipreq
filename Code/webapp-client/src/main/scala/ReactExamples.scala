...
object ReactExamples {
  ...
  def main(routines: Routines.ForCfgReqType) = IO[Unit] {
    ...
    ClientData.init(routines.projectInit, clientData => IO {
      CfgReqType.Component(CfgReqType.Props((routines.reqCrud, clientData), false)) render document.getElementById("eg2")
      CfgIncmpType.Component(CfgIncmpType.Props((routines.incmpCrud, clientData), false)) render document.getElementById("eg3")
    }).unsafePerformIO()
  }
