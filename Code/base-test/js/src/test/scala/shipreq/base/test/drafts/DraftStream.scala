...
//    ???
  ...
  def genAndApplyDiff(t: Yjs.YText, from: String, to: String): Unit = {
    (from, to) match {
      case ("[event 3]", "[event 3]") => t.replaceFirst("3", "5")
      case ("[event 3]", "[event 4]") => t.replaceFirst("3", "4")
      case ("[event 4]", "[event 5]") => t.replaceFirst("4", "5")
      case _ => throw new RuntimeException(s"genAndApplyDiff: '$from' -> '$to'")
    }
  }
