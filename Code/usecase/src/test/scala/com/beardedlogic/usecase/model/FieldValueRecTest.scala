...
class FieldValueRecTest extends FunSpec with TestHelpers {
  ...
  val FK1 = new FieldKeyRec(11, FieldKeyType.Text, Some("Ah1"))
  val FK2 = new FieldKeyRec(12, FieldKeyType.Text, Some("Ah2"))
  val FK3 = new FieldKeyRec(13, FieldKeyType.Text, Some("Ah3"))
  ...
  val Step1 = new PlainValue[DataType.Step](41,51,1)
  val Step2 = new PlainValue[DataType.Step](42,52,1)
  val Step3 = new PlainValue[DataType.Step](43,53,1)

  describe("Combining FieldSaveCtx") {
    it("should combine all maps") {
      val a = FieldSaveCtx(Map(FK1 -> FV1), SVMap("s1" -> Step1))
      val b = FieldSaveCtx(Map(FK2 -> FV2), SVMap("s2" -> Step2))
      val c = a.combineWith(b)
      c.fieldValues should be(Map(FK1 -> FV1, FK2 -> FV2))
      c.stepValues should be(SVMap("s1" -> Step1, "s2" -> Step2))
    }

    it("should not override conflicts") {
      val a = FieldSaveCtx(Map(FK1 -> FV1), SVMap("s1" -> Step1))
      val b = FieldSaveCtx(Map(FK1 -> FV2), SVMap("s1" -> Step2))
      val c = a.combineWith(b)
      c.fieldValues should be(Map(FK1 -> FV1))
      c.stepValues should be(SVMap("s1" -> Step1))
    }
  }

  /*
  def getFieldKey(t: FieldKeyType) = Defaults.FieldListRec.fieldKeys.find(_.fieldKeyType == t).get
