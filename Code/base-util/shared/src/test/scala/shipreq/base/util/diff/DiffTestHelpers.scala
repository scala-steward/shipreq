...
object DiffTestHelpers {
  ...
  implicit def autoWrapStrs(s: String): DiffSource[Char] =
  ...
  def assertCharDiff(src: String, tgt: String)(expectedOps: String*)(implicit l: Line, algo: DiffAlgorithm[Char]): Unit = {
  ...
  def assertRoundTrip(src: String, tgt: String)(implicit l: Line, algo: DiffAlgorithm[Char]): Ops = {
  ...
  def propTest(samples: Int = 100)(implicit algo: DiffAlgorithm[Char]): Unit = {
    val prop = Prop.equal[(String, String)]("p(a, Δᵃᵇ) = b")(
      actual = _._2,
      expect = { case (x, y) => applyPatch(x, y, algo.diff(x, y)(Ops)) },
    )
    val genStr = Gen.chooseChar_!('A' to 'H').string(0 to 92)
