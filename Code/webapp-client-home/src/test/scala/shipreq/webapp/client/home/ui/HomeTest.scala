...
object HomeTest extends TestSuite {
  ...
  object Data {
    ...
    val now = Instant.now()
    val piE = ProjectMetaData("abeF", "Empty", 0, 0, 0, 0, now.minus(18, DAYS), None)
    val piO = ProjectMetaData("qwe3F", "Old", 2, 1581, 311, 340, now.minus(92, DAYS), Some(now.minus(7, MINUTES)))
    val piN = ProjectMetaData("wenkj", "New", 2, 2, 0, 0, now, None)
    val pc  = List(piE, piO)
  }
