...
case class EditValidationFeature[+E, +A](value: PotentialChange[E, A])

object EditValidationFeature {
  type Result[+A] = EditValidationFeature[Invalidity, A]

  @inline implicit def delegatePotentialChange[E, A](f: EditValidationFeature[E, A]): PotentialChange[E, A] =
    f.value

  def compare[A](vr: Invalidity \/ A)(previous: A)(implicit e: Equal[A]): Result[A] =
    apply(
      PotentialChange.fromDisjunction(vr)
  ...
  def setDiff[A](vr: Invalidity \/ SetDiff[A]): Result[SetDiff.NE[A]] =
    apply(PotentialChange.fromDisjunction(vr).flatMap(PotentialChange nonEmpty _))
